...
#ifdef _OPENMP
        std::cout << "[INFO] OpenMP voting enabled. vote_threads="
                  << ((args.vote_threads > 0) ? args.vote_threads : omp_get_max_threads()) << "\n";
#else
        std::cout << "[INFO] OpenMP not enabled. Build with -fopenmp to use voting parallelism.\n";
#endif
        std::cout << "[INFO] Parallel mode: VOTING ONLY (image loop is sequential)\n";
