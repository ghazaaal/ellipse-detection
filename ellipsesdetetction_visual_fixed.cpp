...
static cv::Mat draw_detected_red_on_original(const cv::Mat& gray, const std::vector<EllipseParams>& preds) {
    cv::Mat out;
    ...
    for (const auto& e : preds) {
        cv::ellipse(
            ...
            0,
            360,
            cv::Scalar(0, 0, 255),   // red in OpenCV BGR
            2,
            cv::LINE_AA
        );
    }
...
static void save_side_by_side_compare(
    const cv::Mat& gray,
    const std::vector<EllipseParams>& preds,
    ...
    cv::Mat original_bgr;
    cv::cvtColor(gray, original_bgr, cv::COLOR_GRAY2BGR);
    cv::Mat detected_bgr = draw_detected_red_on_original(gray, preds);

    if (scale < 1) scale = 1;
    if (scale != 1) {
        cv::resize(original_bgr, original_bgr, cv::Size(), scale, scale, cv::INTER_NEAREST);
        cv::resize(detected_bgr, detected_bgr, cv::Size(), scale, scale, cv::INTER_NEAREST);
    }

    int gap = 10;
    cv::Mat gap_img(original_bgr.rows, gap, CV_8UC3, cv::Scalar(0, 0, 0));
    cv::Mat combined;
    cv::hconcat(std::vector<cv::Mat>{original_bgr, gap_img, detected_bgr}, combined);

    if (!cv::imwrite(out_path, combined)) {
        throw std::runtime_error("Failed to save comparison PNG: " + out_path);
    }
}
    ...
    try {
        ...
        for (size_t idx = 0; idx < image_ids.size(); ++idx) {
            ...
            if (args.save_compare && compare_ids.count(img_id)) {
                ...
                save_side_by_side_compare(loaded.gray, preds, out_png, args.compare_scale);
            }
