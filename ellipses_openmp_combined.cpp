...
static int G_VOTE_THREADS = 0;  // used by voting-level OpenMP version; 0 = OpenMP default
...
struct Args {
    ...
    int compare_scale = 4;
    int num_threads = 0;       // OpenMP image-level threads; 0 = default/max
    int vote_threads = 0;      // OpenMP voting-level threads; 0 = default/max
};
...
static void print_usage() {
    std::cout << "Usage: Batch_Evaluation_DGHT_cpp --ann annotations.json [options]\n"
              ...
              << "  --num_threads INT             OpenMP threads for image-level parallel loop\n"
              << "  --vote_threads INT            OpenMP threads for voting/inlier loop\n";
}
...
static std::vector<uint8_t> boundary_inliers(const std::vector<cv::Point>& xy, const EllipseParams& e, double boundary_tol) {
    ...
    // Voting-level parallelization:
    // each OpenMP thread checks a different subset of bright points
    // for the same candidate ellipse.
    #pragma omp parallel for schedule(static) num_threads(G_VOTE_THREADS > 0 ? G_VOTE_THREADS : omp_get_max_threads()) if(xy.size() > 64)
    for (int i = 0; i < static_cast<int>(xy.size()); ++i) {
        ...
        out[static_cast<size_t>(i)] = (std::abs(v - 1.0) <= boundary_tol) ? 1 : 0;
    }
...
int main(int argc, char** argv) {
    try {
        ...
        G_VOTE_THREADS = (args.vote_threads > 0) ? args.vote_threads : omp_get_max_threads();
        omp_set_nested(1);
        omp_set_max_active_levels(2);
        const int image_threads = (args.num_threads > 0) ? args.num_threads : omp_get_max_threads();
        ...
        #pragma omp parallel for schedule(dynamic) num_threads(image_threads)
        for (int idx = 0; idx < static_cast<int>(image_ids.size()); ++idx) {
            int img_id = image_ids[static_cast<size_t>(idx)];
